...
def run_test(inputs, background, foreground, modality, target, boundary, expected, epsilon):
    '''
    ...
    for iii in range (inputs.len):
        segmentation = find_segmentation(inputs[iii], foreground, background, boundary, target)
        assertLessEqual(compare_results (segmentation, expected[iii]),epsilon)


def find_segmentation(input_img, foreground, background, boundary, target):
    '''
...
def main():
    ...
    run_test(inputs, background, foreground, modality, target, boundary, expected , epsilon)
